...
load("Recs_VTLN_alpha_1_2_04122022_3_25092022.mat")
        ...
        if isempty(all(file).frames_out_Therapist(w).Signal_frame)
            ...
            if ~strcmp( all(file).frames_out_Therapist(w).segment_event ,'Echolalia')
                ...
                if flag == 1
                    ...
                    surf(all(file).frames_out_Therapist(w).t_for_spect,all(file).frames_out_Therapist(w).F_for_spect,...
                        10*log10(all(file).frames_out_Therapist(w).PowerSpectrum),"DisplayName","P_{Spectrum}","EdgeColor","none");
                    view([0,90]); hold on
                    axis([all(file).frames_out_Therapist(w).t_for_spect(1) all(file).frames_out_Therapist(w).t_for_spect(end) all(file).frames_out_Therapist(w).F_for_spect(1) all(file).frames_out_Therapist(w).F_for_spect(end)])
                    ...
                    surf(all(file).frames_out_child(echo_idx).t_for_spect,...
                        all(file).frames_out_child(echo_idx).F_for_spect,10*log10(all(file).frames_out_child(echo_idx).PowerSpectrum),...
                        "DisplayName","Power Spectrum","EdgeColor","none");
                    view([0,90]); hold on
                    axis([all(file).frames_out_child(echo_idx).t_for_spect(1) all(file).frames_out_child(echo_idx).t_for_spect(end) all(file).frames_out_child(echo_idx).F_for_spect(1) all(file).frames_out_child(echo_idx).F_for_spect(end)])
                    xlabel('Time (s)');hold on
...
%     F1_chWa = [F1_chWa all(file).F1Warped(~isnan(all(file).F1Warped)) ];
%     F2_chWa = [F2_chWa all(file).F2Warped(~isnan(all(file).F2Warped)) ];
%     F3_chWa = [F3_chWa all(file).F3Warped(~isnan(all(file).F3Warped)) ];
