...
Param.alpha=15/16; % for pre emphasis0.63
...
alpha = 1.08; % for frequency warping 0.88 -1.12
...
for file = 3:length(all)
  ...
  if isempty(fieldnames(frames_out_child)) || isempty(fieldnames(frames_out_Therapist))
    continue
  end
      ...
      if ~strcmp( frames_out_Therapist(w).segment_event ,'Echolalia')
        ...
        for j = 1:length(frames_out_child)
          if isempty(frames_out_child(j).t)
            ...
            if (frames_out_Therapist(w).t(end)+40) >(frames_out_child(j).t(1)) &&...
        ...
        if flag == 1
          ...
          surf(frames_out_Therapist(w).t_for_spect,frames_out_Therapist(w).F_for_spect,...
            10*log10(frames_out_Therapist(w).PowerSpectrum),"DisplayName","P_{Spectrum}","EdgeColor","none");
          view([0,90]); hold on
          axis([frames_out_Therapist(w).t_for_spect(1) frames_out_Therapist(w).t_for_spect(end) frames_out_Therapist(w).F_for_spect(1) frames_out_Therapist(w).F_for_spect(end)])
          ...
          surf(frames_out_child(echo_idx).t_for_spect,...
            frames_out_child(echo_idx).F_for_spect,10*log10(frames_out_child(echo_idx).PowerSpectrum),...
            "DisplayName","Power Spectrum","EdgeColor","none");
          view([0,90]); hold on
          axis([frames_out_child(echo_idx).t_for_spect(1) frames_out_child(echo_idx).t_for_spect(end) frames_out_child(echo_idx).F_for_spect(1) frames_out_child(echo_idx).F_for_spect(end)])
          xlabel('Time (s)');hold on
