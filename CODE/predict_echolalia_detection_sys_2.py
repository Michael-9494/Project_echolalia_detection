...
if __name__ == '__main__':
    VTLN = "no_VTLN"   # Identifier for VTLN process # 
    date = "26_Jul_2023" # Date for the current run
    ...
    read_path_aug = "C:\\Users\\97254\\Documents\\GitHub\\Project-speech\\Project_echolalia_detection\\py\\py-part\\CODE\\augment\\aug_pairs\\"
    ...
    record_arr_all_p = pd.DataFrame(record_arr_all, columns='record'.split())
    
    # Load and reshape augmented training data
    Mels_train_aug = np.load(read_path_aug+"mel_aug_"+VTLN + ".npy")
    Mels_train_aug = np.reshape(Mels_train_aug, (Mels_train_aug.shape[0],
                                                 Mels_train_aug.shape[2],
                                                 Mels_train_aug.shape[3],
                                                 Mels_train_aug.shape[1]))
    y_train_aug = np.load(read_path_aug+"echo_list_aug_all.npy")
    
    # Load record arrays for training and augmentation
    
    record_arr_aug_all = np.load(read_path_aug+"record_arr_aug_all.npy")
    record_arr_all_p_aug = pd.DataFrame(
        record_arr_aug_all, columns='record'.split())
    record_arr_all_p_aug['record'] = record_arr_all_p_aug['record']
    ...
    for fold_i, (train_val_index, test_index) in enumerate(skf.split(Child_Key, Echo_score)):
        ...
        logic_record_all_train = record_arr_all_p['record'].isin(train_data[0])
        logic_record_all_train_aug = record_arr_all_p_aug['record'].isin(
            train_data[0])

        logic_record_all_val = record_arr_all_p['record'].isin(val_data[0])

        logic_record_all_test = record_arr_all_p['record'].isin(test_data[0])

        x_train_1 = np.concatenate((Mels_train[logic_record_all_train, :, :, 0],
                                   Mels_train_aug[logic_record_all_train_aug, :, :, 0]),
                                   axis=0)

        x_train_2 = np.concatenate((Mels_train[logic_record_all_train, :, :, 1],
                                   Mels_train_aug[logic_record_all_train_aug, :, :, 1]),
                                   axis=0)

        y_train = np.concatenate(
            (y_train_0[logic_record_all_train],
             y_train_aug[logic_record_all_train_aug]), axis=0)
        
        # # Create the new vector filled with zeros
        
        # Create a new vector with zeros
        aug_track = np.zeros(len(y_train))
        for iii in range(len(y_train_0[logic_record_all_train])):
            if y_train[iii]==0:
                aug_track[iii] = 0
            else:
                aug_track[iii] = 2
        for iii in range(len(y_train_0[logic_record_all_train]),
                         len(y_train_aug[logic_record_all_train_aug])+
                         len(y_train_0[logic_record_all_train])):
            
            aug_track[iii] = 1
                  
            
        sum_aug_track_2 = np.sum(aug_track == 2)
        sum_aug_track_1 = np.sum(aug_track == 1)
        sum_aug_track_0 = np.sum(aug_track == 0)
        print('sum_aug_track_0 (no echo)  '+f"{(sum_aug_track_0)}")
        print('sum_aug_track_1 (aug echo) '+f"{(sum_aug_track_1)}")
        print('sum_aug_track_2 (True echo)'+f"{(sum_aug_track_2)}")
        # print('sum_aug_track_0 (no echo)  '+f"{sum(aug_track_0)}")
        # print('sum_aug_track_1 (aug echo) '+f"{sum(aug_track_1)}")
        # print('sum_aug_track_2 (True echo)'+f"{sum(aug_track_2)}")
        # print(aug_track)
        
        aug_list.append(aug_track)
        y_train_record_name = pd.concat([
            record_arr_all_p['record'][logic_record_all_train],
            record_arr_all_p_aug['record'][logic_record_all_train_aug]
        ])
        ...
        (results,
         history,
         siamese) = Functions.run_siamese_network(x_train_1, x_train_2, y_train,
        ...
        best_PPV_list_from_tresh_train.append(PPV_train[np.argmax(UAR_train)])
    ...
    conc_arr['prediction_sys_2_'+VTLN+'_UAR'] = concatenated_pred_UAR.values
    conc_arr['prediction_sys_2_'+VTLN+'_PPV'] = concatenated_pred_PPV.values
    ...
    concatenated_True_echo_train = np.concatenate([(True_echo_list_train[0]),
                                                  ...
                                                  axis=0)
    concatenated_aug_list_train = np.concatenate([(aug_list[0]),
                                                  (aug_list[1]),
                                                  (aug_list[2]),
                                                  (aug_list[3]),
                                                  (aug_list[4])],
    ...
    conc_arr_train['aug_list_train_'+VTLN ] = concatenated_aug_list_train

    conc_arr_train['prediction_train_sys_2_'+VTLN +
                   '_PPV'] = concatenated_pred_PPV_train.values

    conc_arr_train['fold_train_sys_2_'+VTLN] = concatenated_score_fold_train
