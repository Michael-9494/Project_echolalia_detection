...
if __name__ == '__main__':
    ...
    date = "28_Jul_2023"  # Date for the current run
    ...
    for VTLN in VTLNs:
        ...
        (predictions_net_test, predictions_net_train,
         predictions_net_val) = [], [], []
        ...
        for fold_i, (train_val_index, test_index) in enumerate(
                skf.split(Child_Key, Echo_score)):
            ...
            train_idx, val_idx = next(
                skf1.split(X=train_val_data.reset_index(drop=True),
            ...
            (results, history, siamese) = Functions.run_siamese_network(
                x_train_1, x_train_2, y_train,
                x_val_1, x_val_2, y_val,
                x_test_1, x_test_2, y_test,
                fold_i, save_path, param_grid,
                VTLN, date)
            ...
            predictions_net_test.append(Predictions)
            ...
            (sensitivity_train, one_minus_specificity_train,
             ...
             best_threshold_PPV_train) = Functions.find_stuff(
            ...
            ax_train.plot(one_minus_specificity_train, sensitivity_train, lw=1.2,
                          alpha=0.4,
                          label=f" ROC fold {fold_i} (AUC={AUC_train:.3f}) ")
            ...
            predictions_net_train.append(Predictions_train)
            ...
            (sensitivity_val, one_minus_specificity_val,
             ...
             best_threshold_PPV_val) = Functions.find_stuff(
            ...
            ax_val.plot(one_minus_specificity_val, sensitivity_val, lw=1.2,
                        alpha=0.4,
                        label=f" ROC fold {fold_i} (AUC={AUC_val:.3f})")
            ...
            predictions_net_val.append(Predictions_val)
        ...
        con_predictions_net_test = np.concatenate([(predictions_net_test[0]),
                                                   (predictions_net_test[1]),
                                                   (predictions_net_test[2]),
                                                   (predictions_net_test[3]),
                                                   (predictions_net_test[4])],
                                                  axis=0)
        con_pred_UAR = pd.concat([
        ...
        con_rec = pd.concat([predictions_list_rec[0],
                             predictions_list_rec[1],
                             predictions_list_rec[2],
                             predictions_list_rec[3],
                             predictions_list_rec[4]], axis=0)

        con_score_echo = np.concatenate([(score_echo[0]),
                                         (score_echo[1]),
                                         (score_echo[2]),
                                         (score_echo[3]),
                                         (score_echo[4])], axis=0)

        con_True_echo_test = np.concatenate([(True_echo_list_test[0]),
                                             (True_echo_list_test[1]),
                                             (True_echo_list_test[2]),
                                             (True_echo_list_test[3]),
                                             (True_echo_list_test[4])],
                                            axis=0)
        con_score_fold = np.concatenate([(fold_list_test[0]),
                                         (fold_list_test[1]),
                                         (fold_list_test[2]),
                                         (fold_list_test[3]),
                                         (fold_list_test[4])], axis=0)

        conc_arr['record'] = con_rec.values
        conc_arr['True_echo'] = con_True_echo_test
        conc_arr['prediction_val_sys_2_' +
                 VTLN] = con_predictions_net_test
        conc_arr['prediction_sys_2_'+VTLN +
                 '_UAR'] = con_pred_UAR.values
        conc_arr['prediction_sys_2_'+VTLN +
                 '_PPV'] = con_pred_PPV.values

        conc_arr['score_echo_sys_2_'+VTLN] = con_score_echo
        conc_arr['fold_test_sys_2_'+VTLN] = con_score_fold
        ...
        Functions.make_report(cm_list_UAR, best_UAR_list,
                              best_threshold_list_UAR,
                              cm_list_PPV, best_PPV_list,
                              best_PPV_list_from_tresh,
                              best_threshold_list_PPV, save_path, save_name)
        # siamese.summary()
        '''
        save train results
        '''
        con_predictions_net_train = np.concatenate([(predictions_net_train[0]),
                                                   (predictions_net_train[1]),
                                                   (predictions_net_train[2]),
                                                   (predictions_net_train[3]),
                                                   (predictions_net_train[4])],
                                                   axis=0)
        con_pred_UAR_train = pd.concat([
        ...
        con_rec_train = pd.concat([predictions_list_rec_train[0],
        ...
        con_score_echo_train = np.concatenate([(score_echo_train[0]),
                                               (score_echo_train[1]),
                                               (score_echo_train[2]),
                                               (score_echo_train[3]),
                                               (score_echo_train[4])],
                                              axis=0)
        con_score_fold_train = np.concatenate([(fold_list_train[0]),
                                               (fold_list_train[1]),
                                               (fold_list_train[2]),
                                               (fold_list_train[3]),
                                               (fold_list_train[4])],
                                              axis=0)
        con_True_echo_train = np.concatenate([(True_echo_list_train[0]),
        ...
        conc_arr_train['record_train'] = con_rec_train.values
        conc_arr_train['True_echo_train'] = con_True_echo_train
        conc_arr_train['prediction_val_sys_2_' +
                       VTLN] = con_predictions_net_train

        conc_arr_train['prediction_train_sys_2_'+VTLN +
                       '_UAR'] = con_pred_UAR_train.values

        conc_arr_train['prediction_train_sys_2_'+VTLN +
                       '_PPV'] = con_pred_PPV_train.values

        conc_arr_train['fold_train_sys_2_' +
                       VTLN] = con_score_fold_train

        conc_arr_train['score_train_echo_sys_2_' +
                       VTLN] = con_score_echo_train
        ...
        Functions.plot_ROC(ax_train, tprs_AUC_train, aucs_AUC_train, save_path,
                           ...
                           y_lable="sensitivity", x_lable="1-Specificity",
                           mean_tpr_1=1.0, x=[1, 0], y=[1, 0],
                           title=" train ROC " + VTLN)

        save_name = "ROC_curve_train_sys_2_PPV_" + VTLN
        Functions.plot_ROC(ax2_train, tprs_PPV_train, aucs_PPV_train,
                           save_path, save_name,
                           y_lable="PPV", x_lable="sensitivity",
                           mean_tpr_1=0.0, x=[1, 0], y=[1, 0],
                           title=" train ROC " + VTLN)
        ...
        Functions.make_report(cm_list_UAR_train, best_UAR_list_train,
                              ...
                              best_threshold_list_PPV_train,
                              save_path, save_name)
        '''
        save validation results
        '''
        con_predictions_net_val = np.concatenate([(predictions_net_val[0]),
                                                  (predictions_net_val[1]),
                                                  (predictions_net_val[2]),
                                                  (predictions_net_val[3]),
                                                  (predictions_net_val[4])],
                                                 axis=0)
        con_pred_UAR_val = pd.concat([
        ...
        con_rec_val = pd.concat([predictions_list_rec_val[0],
        ...
        con_score_echo_val = np.concatenate([(score_echo_val[0]),
                                             (score_echo_val[1]),
                                             (score_echo_val[2]),
                                             (score_echo_val[3]),
                                             (score_echo_val[4])],
                                            axis=0)
        con_score_fold_val = np.concatenate([(fold_list_val[0]),
                                             (fold_list_val[1]),
                                             (fold_list_val[2]),
                                             (fold_list_val[3]),
                                             (fold_list_val[4])],
                                            axis=0)
        con_True_echo_val = np.concatenate([(True_echo_list_val[0]),
        ...
        conc_arr_val['record_val'] = con_rec_val.values
        conc_arr_val['True_echo_val'] = con_True_echo_val
        conc_arr_val['prediction_val_sys_2_'+VTLN] = con_predictions_net_val
        conc_arr_val['prediction_val_sys_2_'+VTLN +
                     '_UAR'] = con_pred_UAR_val.values
        conc_arr_val['prediction_val_sys_2_'+VTLN +
                     '_PPV'] = con_pred_PPV_val.values
        conc_arr_val['fold_val_sys_2_'+VTLN] = con_score_fold_val
        conc_arr_val['score_val_echo_sys_2_' +
                     VTLN] = con_score_echo_val
        ...
        Functions.make_report(cm_list_UAR_val, best_UAR_list_val,
                              ...
                              best_threshold_list_PPV_val, save_path,
                              save_name)


conc_arr.index = con_rec.index
conc_arr = conc_arr.sort_index()
save_name = "concatenated_sys_2_test.csv"
conc_arr.to_csv(save_path+save_name)

conc_arr_train.index = con_rec_train.index
conc_arr_train = conc_arr_train.sort_index()
save_name = "concatenated_sys_2_train.csv"
conc_arr_train.to_csv(save_path+save_name)

conc_arr_val.index = con_rec_val.index
conc_arr_val = conc_arr_val.sort_index()
save_name = "concatenated_sys_2_val.csv"
