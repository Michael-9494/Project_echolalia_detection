...
nx=length(x(:));% the signal length
nwin=length(win);% the window's length
if (nwin == 1) % if win is scalar
   len = win;
else
   len = nwin;
end
...
nf = fix((nx-len+inc)/inc);% the number of segmants
f=zeros(nf,len); % the segmants matrix
indf= inc*(0:(nf-1)).';% the initial index of the segment
inds = (1:len);% numbers between 1-the segment length
